...
*/
		int my_rank, num_procs, thread_status;
		...
		//MPI_Init(&argc, &argv);
	        ...
	        //#pragma omp parallel for private(x, y, x1, y1, x2, y2) shared(y_max, y_min, x_max, x_min)
		for ( i = my_rank; i < m; i+=num_procs )
		{
			y = ( ( double ) (     i - 1 ) * y_max   
				+ ( double ) ( m - i     ) * y_min ) 
			/ ( double ) ( m     - 1 );
			#pragma omp parallel for private(x,x1, y1, x2, y2) shared(y_max, y_min, x_max, x_min)
			for ( j = 0; j < n; j++ )
			{
				x = ( ( double ) (     j - 1 ) * x_max   
					+ ( double ) ( n - j     ) * x_min ) 

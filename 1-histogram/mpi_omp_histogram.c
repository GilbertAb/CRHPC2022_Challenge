...
void Histogram(PPMImage *image, float *h, int argc, char *argv[]) {

	int i, j, k, l, x, count, totalCount, meu_ranque, num_procs, raiz = 0;
	int rows, cols;
	int thread_status;
	int n = image->y * image->x;  /* Total de pixels da imagem */
	...
	x = 0;
        ...
        #pragma omp parallel for collapse(3) private(j,k,l)
	for (j = 0; j <= 3; j++) {
		for (k = 0; k <= 3; k++) {
			for (l = 0; l <= 3; l++) {
				...
				MPI_Reduce(&count, &totalCount, 1, MPI_INT, MPI_SUM, raiz, MPI_COMM_WORLD);
